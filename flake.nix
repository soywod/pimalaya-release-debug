      ...
      crossBuildTargets = {
        ...
        x86_64-darwin = rec {
          ...
          aarch64-apple-darwin = pkgs: rec {
            ...
            TARGET_CC = with pkgs.pkgsCross; "${builtins.trace aarch64-darwin.stdenv.cc aarch64-darwin.stdenv.cc}/bin/cc";
            # CARGO_BUILD_RUSTFLAGS = with pkgs.pkgsCross; [ "-C" "linker=${builtins.trace "${aarch64-darwin.stdenv.cc}/bin" TARGET_CC}" ];
            postInstall = ''
              cd $out/bin
              mkdir -p {man,completions}
              qemu-aarch64 ./neverest man ./man
